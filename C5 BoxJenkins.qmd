...
# ── load S&P 500 E-mini Futures (ES=F) from CSV ───────────────────────────────
# Data previously downloaded from Yahoo Finance (ES=F, adjusted close,
# 2025-04-01 to 2025-12-31) and saved to data/es_futures.csv for reproducibility.
snp_raw <- read.csv("data/es_futures.csv")
snp     <- xts(snp_raw$adjusted, order.by = as.Date(snp_raw$date))
colnames(snp) <- "ES=F.Adjusted"
snp     <- na.omit(snp)
...
p_pos <- ggAcf(ar1_pos, lag.max = 20) +
  labs(title = expression(phi[1] == 0.7 ~ " (positive)")) + theme_minimal(base_size = 12)
p_neg <- ggAcf(ar1_neg, lag.max = 20) +
  labs(title = expression(phi[1] == -0.7 ~ " (negative)")) + theme_minimal(base_size = 12)
...
p1 <- ggAcf(sim_ar1,  lag.max=20) + labs(title="AR(1) — ACF")  + theme_minimal(base_size = 12)
p2 <- ggPacf(sim_ar1, lag.max=20) + labs(title="AR(1) — PACF") + theme_minimal(base_size = 12)
p3 <- ggAcf(sim_ma1,  lag.max=20) + labs(title="MA(1) — ACF")  + theme_minimal(base_size = 12)
p4 <- ggPacf(sim_ma1, lag.max=20) + labs(title="MA(1) — PACF") + theme_minimal(base_size = 12)
p5 <- ggAcf(sim_arma,  lag.max=20) + labs(title="ARMA(1,1) — ACF")  + theme_minimal(base_size = 12)
p6 <- ggPacf(sim_arma, lag.max=20) + labs(title="ARMA(1,1) — PACF") + theme_minimal(base_size = 12)
...
We demonstrate the full four-stage Box-Jenkins procedure using **S&P 500 E-mini Futures (ES=F)** daily adjusted close prices, April–December 2025. The series was originally sourced from Yahoo Finance and is loaded here from a saved CSV file (`data/es_futures.csv`) for reproducibility.
...
p_acf_snp  <- ggAcf(est_snp,  lag.max = 30) +
  labs(title = "ACF — SNP Level") + theme_minimal(base_size = 12)
p_pacf_snp <- ggPacf(est_snp, lag.max = 30) +
  labs(title = "PACF — SNP Level") + theme_minimal(base_size = 12)
...
p_acf_d  <- ggAcf(diff_snp,  lag.max = 30, na.action = na.omit) +
  labs(title = "ACF — First Difference") + theme_minimal(base_size = 12)
p_pacf_d <- ggPacf(diff_snp, lag.max = 30, na.action = na.omit) +
  labs(title = "PACF — First Difference") + theme_minimal(base_size = 12)
...
p_ra  <- ggAcf(resid211$.resid,  lag.max = 20, na.action = na.omit) +
  labs(title = "Residual ACF")  + theme_minimal(base_size = 12)
p_rpa <- ggPacf(resid211$.resid, lag.max = 20, na.action = na.omit) +
  labs(title = "Residual PACF") + theme_minimal(base_size = 12)
...
p_acf_w  <- ggAcf(wt,  lag.max = 48) +
  labs(title = "ACF — Stationary Series") + theme_minimal(base_size = 12)
p_pacf_w <- ggPacf(wt, lag.max = 48) +
  labs(title = "PACF — Stationary Series") + theme_minimal(base_size = 12)
