...
# Number of valid forecast pairs per method (n=6 observations)
n_pairs <- c(
  sum(!is.na(F_naive)),   # Naive:        5
  sum(!is.na(F_trend)),   # Naive Trend:  4
  sum(!is.na(F_ac)),      # Avg Change:   3
  sum(!is.na(F_apc)),     # Avg Pct:      3
  sum(!is.na(F_ses[-1])), # SES:          5 (exclude init row)
  sum(!is.na(F_des)),     # DES:          5
  sum(!is.na(F_holt)),    # Holt:         5
  sum(!is.na(F_ar[-1]))   # ARRES:        5 (exclude init row)
)
...
data.frame(
  ...
  knitr::kable(col.names = c("Method","Handles Trend","Handles Seasonal",
                              "Multi-step","Valid Pairs","MSE (example)"),
               align = "c")
```
