...
Set up the R programming environment for Chapter 1.
...
p1 <- ggplot(data.frame(t, y = 10 + 0.5*t + rnorm(60, 0, 1.5)),
      ...
      labs(title="Upward Trend", x="Time", y="Value") + theme_minimal(base_size = 12)

p2 <- ggplot(data.frame(t, y = 40 - 0.5*t + rnorm(60, 0, 1.5)),
             aes(t, y)) + geom_line(colour="#4575b4", linewidth=0.8) +
      labs(title="Downward Trend", x="Time", y="Value") + theme_minimal(base_size = 12)

p3 <- ggplot(data.frame(t, y = 25 + rnorm(60, 0, 1.5)),
             aes(t, y)) + geom_line(colour="#1a9641", linewidth=0.8) +
      labs(title="No Trend / Flat Trend", x="Time", y="Value") + theme_minimal(base_size = 12)

p4 <- ggplot(data.frame(t, y = 10 * exp(0.04*t) + rnorm(60, 0, 3)),
             aes(t, y)) + geom_line(colour="#fc8d59", linewidth=0.8) +
      labs(title="Non-linear (Exponential) Trend", x="Time", y="Value") + theme_minimal(base_size = 12)
...
ggplot(df_u, aes(x = Year, y = u_rate)) +
  ...
  labs(title = "Turning Points in Malaysia's Unemployment Rate (1982-2020)",
       subtitle = "Red triangle = Peak (reverses down)   |   Green triangle = Trough (reverses up)",
...
p2 <- ggplot(na.omit(resid_df), aes(x = Residual)) +
  ...
  labs(title = "Distribution of Residuals",
       ...
       x = expression(I[t]), y = "Density") + theme_minimal(base_size = 12)
...
# kableExtra styling (row highlight) only supports HTML/LaTeX, not Word.
# Keep the styled table for HTML and PDF; plain editable table for docx.
