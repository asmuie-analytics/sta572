...
| **Horizontal/Flat** | Values remain relatively stable |
...
- Provides **insights into underlying patterns and relationships** — useful for decision-making.
...
- If $k = 3$: **3-period moving average**
- If $k = 5$: **5-period moving average**
...
ggplot(data = electric, aes(x = date, y = index)) +
  ...
  labs(title = "Monthly Electricity Output Index (Jan 2015 – Dec 2023)",
...
```{r}
#| label: fig-outliers
#| fig-cap: "Outlier detection in oil filter sales using Z-score method (|z| > 2)"
#| fig-height: 4.5

data(oilfilters)
oil_f <- data.frame(Date  = as.numeric(time(oilfilters)),
                    Sales = as.numeric(oilfilters))
oil_f$z    <- as.numeric(scale(oil_f$Sales))
oil_f$flag <- abs(oil_f$z) > 2
oil_f$label <- ifelse(oil_f$flag, paste0("z = ", round(oil_f$z, 2)), NA)

ggplot(oil_f, aes(x = Date, y = Sales)) +
  geom_line(colour = "grey50", linewidth = 0.7) +
  geom_point(aes(colour = flag, size = flag)) +
  geom_text(aes(label = label), vjust = -1.2, size = 3,
            colour = "#d73027", na.rm = TRUE) +
  scale_colour_manual(values = c("FALSE" = "steelblue", "TRUE" = "#d73027"),
                      labels = c("FALSE" = "Normal", "TRUE" = "Potential Outlier")) +
  scale_size_manual(values = c("FALSE" = 1.5, "TRUE" = 3.5)) +
  labs(title = "Oil Filter Sales with Z-score Outlier Detection",
       subtitle = "Points with |z| > 2 are flagged as potential outliers",
       x = "Year", y = "Sales", colour = NULL, size = NULL) +
  theme_ts()
```

```{r}
#| label: outlier-detection
z       <- scale(as.numeric(oilfilters))
flagged <- which(abs(z) > 2)
cat("Flagged observations (|z| > 2):\n")
cat("  Time points :", round(as.numeric(time(oilfilters))[flagged], 3), "\n")
cat("  Sales values:", as.numeric(oilfilters)[flagged], "\n")
cat("  Z-scores    :", round(z[flagged], 3), "\n")
```
...
|---------|---------|-----------|--------------|
| **MSE** | $\frac{1}{n}\sum(y_t - \hat{y}_t)^2$ | Penalises large errors | Sensitive to outliers |
| **MAPE** | $\frac{1}{n}\sum\left\|\frac{y_t - \hat{y}_t}{y_t}\right\| \times 100$ | Scale-independent (%) | Undefined when $y_t = 0$ |
| **MAE** | $\frac{1}{n}\sum\|y_t - \hat{y}_t\|$ | Robust to outliers | Treats all errors equally |

::: {.callout-note}
The **best model** produces the **lowest** error measure value across multiple measures.
:::
...
knitr::kable(
  ...
  col.names = c("Model", "MSE", "MAE", "MAPE (%)"), align = "c"
)
```
